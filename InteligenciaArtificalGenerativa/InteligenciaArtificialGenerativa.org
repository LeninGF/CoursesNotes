...
** Summary
* 2025-01-15 Problemas LLMs
** Questions and keywords
- Aluucinacion ::
** Notes
- problemas de alucinación
- problemas de sezgo
- Para mitigar las alucinaciones es necesario construir prompts completos y específicos
- Dentro de ~evaluate~ existe la librearia ~toxicity~ para identificar lenguaje de odio

#+begin_src python :session :results output :exports both
from evaluate import load
toxicity = load("toxicity")

text1 = ["""....and viciously telling the people of the United States, the greatest and most powerful Nation on earth,
how our government is to be run. Why don’t they go back and help fix the totally broken and crime infested places
from which they came. Then come back and show us how...."""]

toxicity_results_1 = toxicity.compute(predictions=text1, aggregation="maximum")
print(toxicity_results_1)
#+end_src

#+RESULTS:
: Using default facebook/roberta-hate-speech-dynabench-r4-target checkpoint
: {'max_toxicity': 0.042514387518167496}

- Regard sirve para analizar la polarización de un texto
- se recomienda que si el texto es negativo se revise

** Summary

* 2025-01-15 Transformers para Imagenes
** Questions and keywords
** Notes
- requiere preprocesamiento
  - cropping
  - resize
- se usa la biblioteca ~Pillow~ para abrir las imagenes

#+begin_src python :session :results output graphics :exports both
from transformers import image_transforms
from PIL import Image
import numpy as np
import matplotlib.pyplot as plt
original_image = Image.open("./images/elena.jfif")

image_arr = np.array(original_image)
print(image_arr.shape)
cropped_img = image_transforms.center_crop(image=image_arr, size=(100,100))
plt.imshow(cropped_img)
plt.savefig("./images/cropped_elena.png")
print("./images/cropped_elena.png")
#+end_src

#+RESULTS:
: (511, 509, 3)
: ./images/cropped_elena.png
...
** Summary
* 2025-01-22 VAE autoencoder varacional
** Questions and keywords
- zmean ::
- zvar ::
- función de perdida :: que es y a que se refiere ya he visto ese nombre con anterioridad
** Notes
- el encoder mapea la entrada a un vector mean y a un vector varianza
- se mapea el logaritmo de la varianza
- el encoder del vae es una red neuronal
- con esto no solo se tiene una coordenada del espacio latente sino un conjunto de vectores
- la idea es solucionar el problema de los autoencoders tradicionales
- el decoder sigue siendo el mismo que ya se uso
- de esta manera los puntos alrededor de un area producen imagenes similares
- funció
*** Ejercicio
- revisar codigo  y ejecutar
- explicar lo que hace el código
- explicar loque cada parte hace enla tabla presentada ysu diferencia
  wrt al autoencder normal
*** Codigo Fashion
- 
** Summary
...
** Questions and keywords
- espacio latente ::
- que es el morphing de dos caras ::
- generador :: producir datos indistinguilbles de los reales
- discriminador :: predice si son falsos o reales
** Notes
- VAE con faces usa espacio latente de 200 dimensiones por la cantidad
  de detall
- La normalización en los VAE permite que podamos predecir en todo el
  espacio latente
- revisar en el dataset de faces como funciona el smiling para poder
  hacer que un rostro tenga mas sonrisa
- pero esto podria ser que aprenda mas o menos cancer?
- podría aprender a generar imagenes cancerosas futuras?
- El espacio latente permit hacer operaciones matematicas para hace
  runa imagen con caracteristicas nuevas
*** GAN
- aparecen en 2014
- creador Ian Godfellow
- genera datos sinteticos a partir de un vector de ruido
- Tiene un generador
- Tiene un disccrimandor
- juego de suma 0  en dond eel generador trata de confundir al discriminador
- Hay procesos de entrenamiento tanto para el discrimnador y para el geneardor
- un valor cercano a 0, el dato es falso
- tanto el generador ocmo el discriminador incrementan su desempe;o
- discriminador se entrena primero
** Ejercicio
Revisar y explicar el codigo de vae faces
** Summary
